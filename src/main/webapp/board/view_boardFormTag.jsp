	...
	<div class="board_wrap">
		...
		<div class="board_list_wrap">
			...
			<div class="board_list">
				...
				<!-- 게시물이 없을 경우 -->
				<c:if test="${empty hb_list }">
					<div class="text">게시글이 없습니다</div>
				</c:if>
				<!-- 게시물이 있을 경우 -->
				<!-- 현재 게시물 정리 순서는 최신순 정렬 -->
				<c:if test="${not empty hb_list}">
					<c:forEach var="review_board" items="${hb_list }">
						<div class="text">
			...
			<div class="board_page">
				...
				<c:if test="${startPage > PAGE_PER_BLOCK}">
					<a class="bt first"
						onclick="location.href='View_BoardFormTag.wo?pageNum=${pageNum = 1}&review_hb_tag=${review_hb_tag }'"><<</a>
				</c:if>
				<!-- 페이지 블록 넘기기 ( 이전 ) -->
				<c:if test="${startPage > PAGE_PER_BLOCK}">
					<a
						onclick="location.href='View_BoardFormTag.wo?pageNum=${startPage -1}&review_hb_tag=${review_hb_tag }'"
						class="bt prev"><</a>
				</c:if>
				<!-- 페이지 블록 5개 표시 -->
				<c:forEach var="i" begin="${startPage}" end="${endPage}">
					<a
						onclick="location.href='View_BoardFormTag.wo?pageNum=${i}&review_hb_tag=${review_hb_tag }'">${i}</a>
				</c:forEach>
				<!-- 페이지 블록 넘기기 ( 다음 ) -->
				<c:if test="${endPage < totalPage}">
					<a
						onclick="location.href='View_BoardFormTag.wo?pageNum=${endPage + 1}&review_hb_tag=${review_hb_tag }'"
						class="bt naxt">></a>
				</c:if>
				<!-- 마지막 페이지 이동 -->
				<c:if test="${endPage < totalPage}">
					<a
						onclick="location.href='View_BoardFormTag.wo?pageNum=${totalPage}&review_hb_tag=${review_hb_tag }'"
						class="bt last">>></a>
				</c:if>
			</div>
			<div class="bt_wrap">
					...
					<div class="button_base b03_skewed_slide_in">
						<div>
							<input class="box1" type="button" value="등록"
								onclick="location.href ='/Project2/board/View_BoardWriteForm.wo'">
						</div>
						<div></div>
						<div>
